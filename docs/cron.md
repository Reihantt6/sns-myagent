...
- `CronScheduler` (`src/cron/cron-scheduler.ts`) ticks every 60 s (configurable `intervalMs`) and runs any due jobs.
- Cron expressions are standard 5-field (minute hour day-of-month month day-of-week); `src/cron/cron-parser.ts` provides `parseCronExpression`, `cronMatches`, `getNextCronRun`, `describeCron`.
- Jobs persist in SQLite via `CronStore` (`cron-store.ts`).
- Action types: `prompt` (feed a prompt to the agent), `shell` (run a command), `skill` (invoke a skill by name).
...
- A tick that throws logs `Cron tick error:` and the scheduler keeps going on the next interval (see `cron-scheduler.ts` error handling).
- Jobs whose action fails are recorded with their last-run timestamp; the scheduler doesn't crash the process.
