...
/**
 * Detect Android/Termux. The release assets are glibc-linked binaries built
 * on GitHub's ubuntu runners; they cannot exec under Android's bionic libc.
 * npm runs this postinstall on the host, so `process.platform` is still
 * "linux" — the env/path checks below are the reliable signals.
 */
function detectTermux() {
	...
	try {
		const out = execFileSync("uname", ["-o"], { stdio: ["ignore", "pipe", "pipe"], encoding: "utf8", timeout: 3000 });
		if (out.trim().toLowerCase() === "android") return true;
	} catch {}
	return false;
}
...
async function main() {
	// Termux / Android: the prebuilt glibc binary cannot exec under bionic.
	// Skip the download entirely and point the user at the source build.
	if (detectTermux()) {
		process.stderr.write(
			"Termux detected: the prebuilt binary is glibc-linked and incompatible with Android. " +
				"Build from source instead: `git clone https://github.com/Reihantt6/sns-myagent.git && " +
				"cd sns-myagent && bun install && bun run build`. See docs/termux.md.\n",
		);
		return;
	}
