...
First binary release of **sns-myagent** (CLI: `snsagent`).

## Highlights

- Prebuilt standalone binary (`bin/snsagent`, ~92 MB, Linux x64, ELF).
...
- **Binary build pipeline:** `bin/snsagent` produced via `pkg`-style bundling.
...
Binary is committed in the repo at `bin/snsagent` (Linux x64, dynamically linked).
...
```bash
curl -L -o snsagent https://raw.githubusercontent.com/Reihantt6/sns-myagent/v0.1.0/bin/snsagent
chmod +x snsagent
./snsagent --help
```
...
- **Large binary in repo:** `bin/snsagent` is ~92 MB, exceeds GitHub's 50 MB recommendation. `git clone` will pull it every time. Mitigation planned for v0.2.0 (move to Release assets + add `npm` ignore rule for `bin/` on source-only installs).
