...
async function checkForNewVersion(currentVersion: string): Promise<string | undefined> {
	...
	try {
		const response = await fetch("https://registry.npmjs.org/@sns-myagent/cli/latest");
	...
	if (parsed.fork) {
		...
		if (!match) {
			throw new SessionResolutionError(
				...
				"Run `snsagent --resume` without an argument to pick from recent sessions, or `snsagent` to start a new one.",
			);
		}
