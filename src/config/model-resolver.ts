...
export function resolveCliModel(options: {
	...
	if (cliProvider && !provider) {
		return {
			...
			selector: undefined,
			warning: undefined,
			error: `Unknown provider "${cliProvider}". Run "snsagent models" to see available providers/models.`,
		};
	}
	...
	if (!model) {
		...
		return {
			...
			thinkingLevel: undefined,
			warning,
			error: `Model "${display}" not found. Run "snsagent models" to see available models.`,
		};
	}
