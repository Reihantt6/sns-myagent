/**
 * `snsagent auth-broker` — manage the omp credential vault.
 */
...
export default class AuthBroker extends Command {
	static description = "Manage the snsagent auth-broker (credential vault)";
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  snsagent auth-broker serve",
		"# Boot on a non-default port\n  snsagent auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  snsagent auth-broker token",
		"# Rotate the bearer token\n  snsagent auth-broker token --regenerate",
		"# List supported OAuth providers\n  snsagent auth-broker list",
		"# Local login (run on the broker host)\n  snsagent auth-broker login anthropic",
		"# Interactive provider selection\n  snsagent auth-broker login",
		"# Remote login over SSH tunnel\n  snsagent auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  snsagent auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  snsagent auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  snsagent auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  snsagent auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  snsagent auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  snsagent auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("snsagent", "auth-broker", AuthBroker);
			return;
		}
